...
#ifndef MINI_PROJECT_LINEARFUNCTION_HPP
#define MINI_PROJECT_LINEARFUNCTION_HPP
...
class LinearFunction : public BaseFunction<DIM> {
...
public:
     ...
     */
    inline explicit LinearFunction(double bound_x, double bound_y = 100., double bound_z = 100., double n_min = 1.,
                                   double n_max = 3.)
    ...
    [[nodiscard]] inline double calcRefrInd(double x, double y) const {
        return n_min_ + (n_max_ - n_min_) * std::sqrt(x * x / bound_x_ / bound_x_ + y * y / bound_y_ / bound_y_);
    }
    ...
    [[nodiscard]] inline double calcRefrInd(double x, double y, double z) const {
        return n_min_ + (n_max_ - n_min_) * std::sqrt(
                x * x / bound_x_ / bound_x_ + y * y / bound_y_ / bound_y_ + z * z / bound_z_ / bound_z_);
    }
};

#endif //MINI_PROJECT_LINEARFUNCTION_HPP
